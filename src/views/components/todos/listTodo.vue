<template>
    <div>
        ...
        <ul class="list-todo" v-if="pendingTask.length > 0">
            <item v-for="(item, index) in pendingTask" :key="index" :item="item" :on-done="onDoneTask" :on-delete="onDelete"></item>
        </ul>
        <div v-else>
        ...
        <ul class="list-todo" v-if="doneTask.length > 0">
            <item v-for="(item, index) in doneTask" :key="index" :item="item" :on-delete="onDelete" :on-up="onUp" :type="1"></item>
        </ul>
        <div v-else>
...
export default {
    ...
    methods: {
        ...
        onUp(id){
            this.$store.dispatch("todo/updateStatusTask", {id, status: PENDING_TASK});
        }
    }
}
</script>
